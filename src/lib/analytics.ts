...
export const buildAnalyticsSnapshot = (
  ...
  scenarioForecasts.forEach((scenario) => {
    const walletKey = plantationWalletMap.get(scenario.id);
    if (!walletKey) {
      return;
    }
    const walletEntry = walletAggregation.get(walletKey);
    if (!walletEntry) {
      return;
    }
    const baseScenario = scenario.scenarios.find(
      (item) => item.name === "base"
    );
    if (!baseScenario) {
      return;
    }
    walletEntry.forecastTotal += baseScenario.projectedYieldKg;
    walletEntry.forecastSamples += 1;
  });

  const walletPerformance = Array.from(walletAggregation.values())
    .map((wallet) => {
      const avgYield = wallet.yieldSamples
        ? Number((wallet.yieldSum / wallet.yieldSamples).toFixed(1))
        : null;
      const forecastKg = wallet.forecastSamples
        ? Number((wallet.forecastTotal / wallet.forecastSamples).toFixed(1))
        : null;
      const harvestTrend = monthKeys.map((key, index) => ({
        label: monthlyPoints[index]?.label ?? key,
        harvested: wallet.harvestByMonth.get(key) ?? 0,
      }));
      return {
        address: wallet.address,
        totalPlantations: wallet.totalPlantations,
        stageCounts: wallet.stageCounts,
        activeTasks: wallet.activeTasks,
        completedTasks: wallet.completedTasks,
        carbonOffsetTons: Number(wallet.carbonOffsetTons.toFixed(2)),
        treeCount: wallet.treeCount,
        areaHectares: Number(wallet.areaHectares.toFixed(2)),
        avgYieldKg: avgYield,
        forecastKg,
        harvestTrend,
    ...
    .sort((a, b) => b.totalPlantations - a.totalPlantations);
